...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      sort_by: {
        type: 'string',
        enum: ['created_at', 'updated_at', 'product_id', 'id'],
      },
      status: {
        type: 'string',
...
export const handler = async (client: TestWiz, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.inventories.list(body)));
};
