...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'post',
  httpPath: '/v1/customers/{customer_id}/contacts',
};

export const tool: Tool = {
  name: 'create_customers_contacts',
  description:
  ...
  inputSchema: {
    ...
    required: ['customer_id', 'email', 'first_name', 'reference_id'],
    $defs: {
      attribute_request: {
        type: 'object',
        properties: {
...
export const handler = async (client: TestWiz, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.customers.contacts.create(customer_id, body)),
  );
};
