...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/v1/customers/{customer_id}/contacts/{contact_id}',
};

export const tool: Tool = {
  name: 'retrieve_customers_contacts',
  description:
...
export const handler = async (client: TestWiz, args: Record<string, unknown> | undefined) => {
  const { contact_id, jq_filter, ...body } = args as any;
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.customers.contacts.retrieve(contact_id, body)),
  );
};
