...
import { AttributeListParams, AttributeListResponse, Attributes } from './resources/attributes';
import {
  Inventories,
  Inventory,
  InventoryCreateOrUpdateParams,
  InventoryCreateOrUpdateResponse,
  InventoryListParams,
  InventoryListResponse,
} from './resources/inventories';
import {
  Invoice,
  InvoiceCreateParams,
...
import {
  ...
  ContainerInfoRequestV2,
  CustomerConsentRequest,
  CustomerConsentRequestV2,
  OrderAddressDetailResponseV2,
  OrderAddressRequest,
  OrderAddressRequestV2,
  OrderAddressResponse,
  OrderAddressResponseV2,
  OrderChargesRequest,
  OrderChargesRequestV2,
  OrderContactRequest,
  OrderContactRequestV2,
  OrderContactResponse,
  OrderCreateParams,
  OrderCreateResponse,
  OrderLineItemRequest,
  OrderLineItemRequestV2,
  OrderListParams,
  OrderListResponse,
  OrderListResponseV2,
...
import {
  ...
  PriceListRetrieveResponse,
  PriceListUpdateParams,
  PriceListUpdateResponse,
  PriceLists,
} from './resources/price-lists';
import {
  ProductPricing,
  ProductPricingRetrieveResponse,
  ProductPricingUpdateParams,
  ProductPricingUpdateResponse,
  ProductPricings,
} from './resources/product-pricings';
import {
  Attribute,
  ProductCreateParams,
...
import { SalesRepListParams, SalesRepListResponse, SalesReps, User } from './resources/sales-reps';
import {
  Shipment,
...
const environments = {
  production: 'https://api.wizcommerce.com',
  environment_1: 'https://api-staging.sourcerer.tech',
};
...
export interface ClientOptions {
   ...
   * - `production` corresponds to `https://api.wizcommerce.com`
   * - `environment_1` corresponds to `https://api-staging.sourcerer.tech`
   */
   ...
   *
   * Defaults to process.env['TEST_WIZ_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TEST_WIZ_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Test Wiz API.
 */
export class TestWiz {
  apiKey: string;
   ...
   * API Client for interfacing with the Test Wiz API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['TEST_WIZ_API_KEY'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['TEST_WIZ_BASE_URL'] ?? https://api.wizcommerce.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TEST_WIZ_BASE_URL'),
    apiKey = readEnv('TEST_WIZ_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TestWizError(
        "The TEST_WIZ_API_KEY environment variable is missing or empty; either provide it, or instantiate the TestWiz client with an apiKey option, like new TestWiz({ apiKey: 'My API Key' }).",
      );
    }
    ...
    if (baseURL && opts.environment) {
      throw new Errors.TestWizError(
        'Ambiguous URL; The `baseURL` option (or TEST_WIZ_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null',
      );
    }

    this.baseURL = options.baseURL || environments[options.environment || 'production'];
    this.timeout = options.timeout ?? TestWiz.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TEST_WIZ_LOG'), "process.env['TEST_WIZ_LOG']", this) ??
      defaultLogLevel;
  ...
  static TestWizError = Errors.TestWizError;
  ...
  inventories: API.Inventories = new API.Inventories(this);
  invoices: API.Invoices = new API.Invoices(this);
  orders: API.Orders = new API.Orders(this);
  paymentMethods: API.PaymentMethods = new API.PaymentMethods(this);
  priceLists: API.PriceLists = new API.PriceLists(this);
  productPricings: API.ProductPricings = new API.ProductPricings(this);
  products: API.Products = new API.Products(this);
  salesReps: API.SalesReps = new API.SalesReps(this);
  shipments: API.Shipments = new API.Shipments(this);
}

TestWiz.Attributes = Attributes;
TestWiz.Customers = Customers;
TestWiz.Inventories = Inventories;
TestWiz.Invoices = Invoices;
TestWiz.Orders = Orders;
TestWiz.PaymentMethods = PaymentMethods;
TestWiz.PriceLists = PriceLists;
TestWiz.ProductPricings = ProductPricings;
TestWiz.Products = Products;
TestWiz.SalesReps = SalesReps;
TestWiz.Shipments = Shipments;

export declare namespace TestWiz {
  ...
  export {
    ProductPricings as ProductPricings,
